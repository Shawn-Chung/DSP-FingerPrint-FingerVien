...
//	cmos_D0307_init() ;  //指纹头D0307 初始化
	...
	ZLG7290_I2C_Init () ; //按键初始化
	ds1302_init() ;	
	flash_init() ;
	oLCD_Init() ;
	iLCD_Init() ;
	ad7843_init() ;   //触摸屏初始化
	gt23l32s4w_init() ;  //字库初始化
...
{
	IRQ_disable(IRQ_EVT_EXTINT5);//禁止中断	
	is_touch_on = 1 ;
	IRQ_clear(IRQ_EVT_EXTINT5);//清除中断标志
	IRQ_enable(IRQ_EVT_EXTINT5);//使能中断
}
/*===================外部中断6处理程序=============*/
//外部中断6为共用中断：
//1、指纹头CMOS的I2C初始化中断
...
//CMOS在初始化之前要把Is_CMOS_Write_End置0，初始化完要把Is_CMOS_Write_End置1。
...
//以下为实时显示部分和照相功能控制
	if ( Is_CMOS_Write_End & ( !Is_CMOS_Save_Image ) ){
		LCD_SET ; //DSP准备往LCD写数据 设置液晶光标 为实时显示做准备  
...
//			*( uint8_t * ) ( 0x80100000 ) = AL422B_DATA ;
		...
		Is_CMOS_Save_Image = 0 ;
 		...
 		LCD_SAVE_TO_RT ; //CPLD将BED数据线转为输入（此句必须有）
		LCD_RT_DISPLAY ; //开始实时显示
	}
//以上为实时显示部分和照相功能控制
}
...
void gpio_init(void)
{
	...
	GPIO_configArgs(
		...
		0x00000030 //Uint32 gppol //INT4下降沿触发//INT5下降沿触发
	);
}
